...
import styles from './ZCRoadWorkList.less';
...
@connect(({ waitinCompletion, loading }) => ({
	...
	submitting1: loading.effects['waitinCompletion/update_Completion']
}))
@Form.create()

class ZCRoadWorkList extends PureComponent {
	...
	componentDidMount() {
		...
		dispatch({
			type: 'waitinCompletion/waitinCompletion_ZCList',
			payload: {
				pageSize: 10,
			}
		});
  ...
  <Input  />
	...
	render() {
		...
		const {submitting1, loading, ZCWaitinCompletionList, completionSearchList } = this.props;
	...
	render:(text)=>{
		...
		const columns = [
				...
				fixed: 'left', 
				render: (text, record) => {
					return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpCarloadConstructionForm2').length>0
					&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpCarloadConstructionForm2')[0].children.filter(item=>item.name=='修改').length>0 ?
	...
	<Button icon="cloud-download" onClick={() => this.handleUpldExportClick()}>
      ...
      <Card bordered={false}>
        <div className={styles.tableList}>
          ...
          <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>整车待完工清单</div>
          <StandardTable
            scroll={{ x: (fieldArray.length * 100) + 500 }}
								
            loading={loading}
            data={ZCWaitinCompletionList}
            bordered
            columns={columns}
