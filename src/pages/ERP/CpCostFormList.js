...
import {
  Button, Input, Form, Card, Icon, Row, Col, Select, message, Modal
} from 'antd';
...
import styles from './CpCostFormList.less';
      ...
      render() {
        ...
        return (
          ...
          >
            ...
            initialValue: [],
          })(<SearchTableList searchList={CpCostFormSearchList} />)}
            </div>
          </Modal>
        );
      }
      }
@connect(({ cpCostForm, loading ,cpRevocation}) => ({
  ...cpCostForm,
  ...cpRevocation,
  loading: loading.models.cpCostForm,
}))
@Form.create()
class CpCostFormList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpCostForm/cpCostForm_List',
      payload: {
        pageSize: 10,
      }
    });
    dispatch({
      type: 'cpRevocation/CpCostForm_SearchList',
    });
  }

  gotoForm = () => {
    router.push(`/warehouse/process/cp_cost_form_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_cost_form_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpCostForm/cpCostForm_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpCostForm/cpCostForm_Update',
        payload: {
          id: key,
          ...value
        },
      ...
      this.setState({
        ...
        delete newobj.client
          ...
          'client.clientCpmpany':newkey,
				...
				window.open(`/api/Beauty/beauty/cpCostForm/export?${stringify(params)}`);
			  };

  render() {
    const { selectedRows ,array,searchVisible} = this.state;
    const { loading, cpCostFormList ,CpCostFormSearchList } = this.props;
    ...
    const field = [
      ...
      render:(text,res)=>{
        ...
        return isNotBlank(res)&&isNotBlank(res.createBy)&&isNotBlank(res.createBy.name)?res.createBy.name:''
      } 
 		},
 		{
			title: '总金额',        
 		...
 		},
     {
      ...
      align: 'center' ,    
 		...
 		{
			title: '大号',        
			dataIndex: 'assmblyBuild.maxCode',   
      inputType: 'text', 
      align: 'center' ,   
			width: 150,          
			editable:  true  ,      
 		...
 		{
			title: '小号',        
			dataIndex: 'assmblyBuild.minCode',   
      inputType: 'text',
      align: 'center' ,    
			width: 150,          
			editable:  true  ,      
 		...
 		{
			title: '总成分类',        
			dataIndex: 'assmblyBuild.assemblyType',   
      inputType: 'text',   
      align: 'center' , 
			width: 100,          
			editable:  true  ,      
        ...
        <Button icon="cloud-download" onClick={() => this.handleUpldExportClick()}>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>成本单</div>
              <StandardEditTable
