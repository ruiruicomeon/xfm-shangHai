...
@Form.create()
class CpStayFormListDateil extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
    ...
    const columns = [
      {
        ...
        fixed: 'left',
        render: (text, record) => {
          return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpStayForm1').length>0
          && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpStayForm1')[0].children.filter(item=>item.name=='修改')
          .length>0?
            <Fragment>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator}>
                <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>总成待报件清单</div>
              </div>
              <StandardEditTable
...
export default CpStayFormListDateil;