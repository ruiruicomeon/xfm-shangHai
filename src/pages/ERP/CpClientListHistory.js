  ...
  Object.keys(obj)
    ...
    const ImportFile = Form.create()(props => {
      ...
      const propsUpload = {
        ...
        fileList: fileL,
        headers: Stoken,
        action: '/api/Beauty/beauty/cpClientHistory/import',
...
class CpClientList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpClient/get_CpClientList',
      payload: {
        pageSize: 10,
      }
    });
  ...
  render() {
    ...
    const { loading, getCpClientList, cpClientSearchList } = this.props;
    ...
    return (
      ...
      >
        <div className={styles.standardList}>
          {isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'CH').length > 0
							&& JSON.parse(getStorage('menulist')).filter(item => item.target == 'CH')[0].children.filter(item => item.name == '修改')
								.length >0 &&
...
.length>0?
  ...
  <Button icon="plus" type="primary" style={{visibility:'hidden'}}>
                    ...
                    <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>历史客户管理</div>
              </div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 100) + 800 }}
                loading={loading}
                data={getCpClientList}
                bordered
                columns={columns}
