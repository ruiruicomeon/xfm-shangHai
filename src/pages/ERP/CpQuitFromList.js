...
import styles from './CpQuitFromList.less';
      ...
      render() {
        ...
        return (
          ...
          >
            ...
            initialValue: [],
          })(<SearchTableList searchList={CpQuitFromSearchList} />)}
            </div>
          </Modal>
        );
      }
      }
@connect(({ cpQuitFrom, loading ,cpRevocation }) => ({
  ...cpQuitFrom,
  ...cpRevocation,
  loading: loading.models.cpQuitFrom,
}))
@Form.create()
class CpQuitFromList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpQuitFrom/cpQuitFrom_List',
      payload: {
        pageSize: 10,
    ...
    dispatch({
      type: 'cpRevocation/CpQuitFrom_SearchList',
    });
  }

  gotoForm = () => {
    router.push(`/warehouse/process/cp_quit_from_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_quit_from_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpQuitFrom/cpQuitFrom_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpQuitFrom/cpQuitFrom_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleSearchVisible = (fieldsValue) => {
    ...
    handleFieldChange = (value) => {
      ...
      handleUpldExportClick = type => {
        ...
        window.open(`/api/Beauty/beauty/cpQuitFrom/export?${stringify(params)}`);
        };
  ...
  render() {
    ...
    const { loading, cpQuitFromList ,CpQuitFromSearchList} = this.props;
    ...
    const columns = [
       ...
       ...fieldArray,
      {
       ...
       render: (text, record) => {
         ...
         (item=>item.target=='cpQuitFrom').length>0&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpQuitFrom')[0]
        ...
        <div className={styles.tableListOperator}>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>退料单</div>
              <StandardEditTable
