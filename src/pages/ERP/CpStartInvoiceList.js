...
@Form.create()
class CpStartInvoiceList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpStartInvoice/get_CpStartInvoiceLine',
    });
  ...
  handleFieldChange = (value) => {
    ...
    handleUpldExportClick = type => {
      ...
      window.open(`/api/Beauty/beauty/cpStartInvoice/export?${stringify(params)}`);
      };
  ...
  render() {
    ...
    const { loading, cpStartInvoiceList ,getCpStartInvoiceLine } = this.props;
    ...
    const columns = [
       ...
       ...fieldArray,
      {
       ...
       align: 'center' ,
       render: (text, record) => { 
         if ((record.approvals =='0'||record.approvals =='2'||record.approvals =='4')&&(isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpStartInvoice').length>0
         && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpStartInvoice')[0].children.filter(item=>item.name=='删除')
         .length>0)) {
           return <Fragment>
...
export default CpStartInvoiceList;