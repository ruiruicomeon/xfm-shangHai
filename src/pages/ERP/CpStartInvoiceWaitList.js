...
@Form.create()
class getIssueCpStartInvoiceList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpStartInvoice/get_IssueCpStartInvoice_List',
      payload: {
        pageSize: 10,
        formType:1,
        status:0
  ...
  handleCompletionClick = (id) => {
		const { dispatch } = this.props;
		const { formValues ,tpageSize ,tcurrent} = this.state;
		if (isNotBlank(id)) {
			dispatch({
				type: 'cpStartInvoice/cpStartInvoice_completion',
				payload: {
					id
				},
				callback: () => {
  ...
  render() {
    ...
    const { loading, getIssueCpStartInvoiceList ,getCpStartInvoiceLine } = this.props;
    ...
    const field = [
      ...
      editable:  false ,   
      render:(text,res)=>{
        return <a onClick={()=>this.goto(res.first)}>{text}</a>
        }    
 		},
 		{
			title: '单据状态',        
    ...
    const columns = [
        ...
        fixed: 'left',
				render: (text, record) => {
					return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='Issue').length>0
					&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='Issue')[0].children.filter(item=>item.name=='开票').length>0 ?
  <Popconfirm title="是否确认开票?" onConfirm={() => this.handleCompletionClick(record.id)}>
    <a>开票</a>
  </Popconfirm>
					:''
					},
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator} style={{'position':'relative'}}>
                <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>待开票清单</div>
              </div>
              <StandardEditTable
              scroll={{ x: (fieldArray.length * 100) + 500 }}
                loading={loading}
                data={getIssueCpStartInvoiceList}
                bordered
                columns={columns}
...
export default getIssueCpStartInvoiceList;