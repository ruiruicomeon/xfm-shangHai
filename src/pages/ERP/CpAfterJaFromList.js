...
import styles from './CpAfterJaFromList.less';
      ...
      >
        <div>
          {getFieldDecorator('genTableColumn', {
            initialValue: [],
          })(<SearchTableList searchList={CpAfterJaFromSearchList} />)}
        </div>
      </Modal>
    );
  }
}
@connect(({ cpAfterJaFrom, loading, cpRevocation }) => ({
  ...cpAfterJaFrom,
  ...cpRevocation,
  loading: loading.models.cpAfterJaFrom,
}))
@Form.create()
class CpAfterJaFromList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpAfterJaFrom/cpAfterJaFrom_List',
      payload: {
        pageSize: 10,
      }
    });
    dispatch({
      type: 'cpRevocation/CpAfterJaFrom_SearchList',
    });
    dispatch({
      type: 'dict/dict',
      payload: {
  ...
  gotoForm = () => {
    router.push(`/accessories/process/cp_after_ja_from_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/accessories/process/cp_after_ja_from_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpAfterJaFrom/cpAfterJaFrom_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpAfterJaFrom/cpAfterJaFrom_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/cpAfterJaFrom/export?${stringify(params)}`);
    };

  render() {
    const { selectedRows, array, searchVisible } = this.state;
    const { loading, cpAfterJaFromList, CpAfterJaFromSearchList } = this.props;
    ...
    const columns = [
      ...
      ...fieldArray,
      {
        ...
        render: (text, record) => {
            ...
            (item => item.target == 'cpAfterJaFrom').length > 0 && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpAfterJaFrom')[0]
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator} style={{ 'position': 'relative' }}>
                <div style={{ textAlign: 'center', fontWeight: 550, fontSize: 28, marginBottom: '16px' }}>售后结案单</div>
              </div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 100) + 500 }}
                // selectedRows={selectedRows}
                loading={loading}
                data={cpAfterJaFromList}
                bordered
                columns={columns}
...
export default CpAfterJaFromList;