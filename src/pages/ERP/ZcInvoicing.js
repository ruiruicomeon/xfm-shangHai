...
class CpCloseList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({
      type: 'cpstatistics/find_AssemblyInStock',
      payload: params,
    });
  };
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/statisticsReport/exportAssemblyInStock?${stringify(params)}`);
  };

  render() {
    const { selectedRows, modalVisiblepass, selectkhflag } = this.state;
    const { loading, cppjstatisticsList, getByBillList, dispatch ,findAssemblyInStocklist} = this.props;
    const columns = [
      {
        title: '总成编码',        
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                ...
                data={{
                  list:
                    isNotBlank(findAssemblyInStocklist) &&
                    isNotBlank(findAssemblyInStocklist.list) &&
                    findAssemblyInStocklist.list.length > 0
                      ? findAssemblyInStocklist.list
                      : [],
                  pagination: {
                    total:
                      isNotBlank(findAssemblyInStocklist) &&
                      isNotBlank(findAssemblyInStocklist.total) 
                        ? findAssemblyInStocklist.total
                        : 0,
                    current:
                      isNotBlank(findAssemblyInStocklist) &&
                      isNotBlank(findAssemblyInStocklist.current)
                        ? findAssemblyInStocklist.current
                        : 1,
                    pageSize:
                      isNotBlank(findAssemblyInStocklist) &&
                      isNotBlank(findAssemblyInStocklist) &&
                      isNotBlank(findAssemblyInStocklist.pageSize)
                        ? findAssemblyInStocklist.pageSize
                        : 10,
                  },
                }}
                bordered
