...
class FgsAnalyzeFromPurchase extends PureComponent {
  ...
  handleUpldExportClick = type => {
    ...
    window.open(
      `/api/Beauty/beauty/statisticsReport/exportCgAvgPriceBillMaterialDetailCS?${stringify(params)}`
    );
  };
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({
        type: 'cpstatistics/find_CgAvgPriceBillMaterialDetailCS',
        payload: {
          ...values,
          pageSize: 10,
          current: 1,
  ...
  render() {
    ...
    const { loading, dispatch, findCgAvgPriceBillMaterialDetailCSList } = this.props;
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                scroll={{ x: 1000 }}
                loading={loading}
                data={findCgAvgPriceBillMaterialDetailCSList}
                bordered
                columns={columns}
