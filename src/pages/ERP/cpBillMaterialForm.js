...
@connect(({ cpBillMaterial, loading ,cpOneCode,cpTwoCode}) => ({
  ...
  submitting1: loading.effects['cpBillMaterial/cpBillMaterial_Add'],
}))
@Form.create()
class CpBillMaterialForm extends PureComponent {
  ...
  render() {
    const { fileList, previewVisible, previewImage, selectflag, selectdata, selectkhflag, selectkhdata ,orderflag ,searchval,twovals} = this.state;
    const {submitting1, submitting, cpBillMaterialGet ,cpOneCodeList ,cpTwoCodeList ,dispatch }  = this.props;
    const {
            ...
            <Row>
              ...
              <Col lg={12} md={12} sm={24}>
                <FormItem {...formItemLayout} label='一级编码'>
                  <Input style={{width:'50%'}} disabled value={isNotBlank(selectdata)&&isNotBlank(selectdata.code)?selectdata.code:(isNotBlank(cpBillMaterialGet) && isNotBlank(cpBillMaterialGet.oneCode) ? cpBillMaterialGet.oneCode : '')} />
              ...
              <Col lg={12} md={12} sm={24}>
                <FormItem {...formItemLayout} label='二级编码'>
                  <Input style={{width:'50%'}} disabled value={isNotBlank(selectkhdata)&&isNotBlank(selectkhdata.code)?selectkhdata.code:(isNotBlank(cpBillMaterialGet) && isNotBlank(cpBillMaterialGet.twoCode) ? cpBillMaterialGet.twoCode : '')} />
              ...
              <Col lg={12} md={12} sm={24}>
                <FormItem {...formItemLayout} label='状态'>
                  {getFieldDecorator('status', {
					initialValue: isNotBlank(cpBillMaterialGet) && isNotBlank(cpBillMaterialGet.status) ? cpBillMaterialGet.status : '',     
					rules: [
					  {
						required:  true ,   
						message: '请选择状态',
