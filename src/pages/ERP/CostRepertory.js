...
class CpCloseList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({
      type: 'cpstatistics/find_CostSummary',
      payload: params,
    });
  };
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/statisticsReport/exportCostSummary?${stringify(params)}`);
  };

  render() {
    const { selectedRows, modalVisiblepass, selectkhflag } = this.state;
    const { loading, cppjstatisticsList, getByBillList, dispatch ,findCostSummarylist} = this.props;
    const columns = [
        {
         title: '订单编号',        
