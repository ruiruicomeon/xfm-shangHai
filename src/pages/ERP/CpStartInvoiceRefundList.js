...
@Form.create()
class getexitCpStartInvoiceList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpStartInvoice/get_exitCpStartInvoice_List',
      payload: {
        pageSize: 10,
        formType: 3
      }
  ...
  render() {
    ...
    const { loading, getexitCpStartInvoiceList ,getCpStartInvoiceLine } = this.props;
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator} style={{ 'position': 'relative' }}>
                <div style={{ textAlign: 'center', fontWeight: 550, fontSize: 28, marginBottom: '16px' }}>发票退票</div>
              </div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 150)  }}
                loading={loading}
                data={getexitCpStartInvoiceList}
                bordered
                columns={columns}
...
export default getexitCpStartInvoiceList;