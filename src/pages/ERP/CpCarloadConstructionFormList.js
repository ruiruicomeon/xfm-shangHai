...
import {
	Button, Input, Form, Card, Popconfirm, Icon, Row, Col, Select, Modal
} from 'antd';
...
import styles from './CpCarloadConstructionFormList.less';
...
@connect(({ cpCarloadConstructionForm, waitinCompletion, loading }) => ({
	...cpCarloadConstructionForm,
	...waitinCompletion,
	loading: loading.models.cpCarloadConstructionForm,
}))
@Form.create()
class CpCarloadConstructionFormList extends PureComponent {
	state = {
		expandForm: false,
		selectedRows: [],
		formValues: {},
	...
	componentDidMount() {
		...
		dispatch({
			type: 'cpCarloadConstructionForm/cpCarloadConstructionForm_List',
			payload: {
				pageSize: 10,
			}
		});
	...
	gotoForm = () => {
		router.push(`/business/process/cp_carload_construction_form_form`);
	}

	gotoUpdateForm = (id) => {
		router.push(`/business/process/cp_carload_construction_form_form?id=${id}`);
	}
	...
	handleDeleteClick = (id) => {
		...
		if (isNotBlank(id)) {
			dispatch({
				type: 'cpCarloadConstructionForm/cpCarloadConstructionForm_Delete',
				payload: {
					id
				},
				callback: () => {
	...
	onSaveData = (key, row) => {
		...
		if (isNotBlank(value)) {
			...
			dispatch({
				type: 'cpCarloadConstructionForm/cpCarloadConstructionForm_Update',
				payload: {
					id: key,
					...value
				},
  ...
  <Input  />
	...
	render() {
		const { selectedRows, array, orderStatus, searchVisible, orderType, business_type, old_need, area, quality_need, oils_need, guise_need, is_photograph, installation_guide, business_project, business_dicth, maintenance_project } = this.state;
		const { loading, cpCarloadConstructionFormList, completionSearchList } = this.props;
  ...
  <Fragment>
    <a onClick={() => this.gotoUpdateForm(record.id)}>详情</a>
			...
			...fieldArray,
			{
				...
				width: 100,
				render: (text, record) => {
					return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpCarloadConstructionForm').length > 0
						&& JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpCarloadConstructionForm')[0].children.filter(item => item.name == '查看').length > 0 ?
	...
	<Button icon="cloud-download" onClick={() => this.handleUpldExportClick()}>
      ...
      <Card bordered={false}>
        <div className={styles.tableList}>
          ...
          <div style={{ textAlign: 'center', fontWeight: 550, fontSize: 28, marginBottom: '16px' }}>整车施工单</div>
          <StandardEditTable
            scroll={{ x: (fieldArray.length * 100) + 500 }}
            loading={loading}
            data={cpCarloadConstructionFormList}
            bordered
            columns={columns}
...
export default CpCarloadConstructionFormList;