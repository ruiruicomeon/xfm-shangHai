...
class FgsAnalyzeFromPurchase extends PureComponent {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            <FormItem label="物料名称">
              {getFieldDecorator('cpBillMaterial', {
                initialValue: '',
              })(<Input />)}
            </FormItem>
          </Col>
  ...
  handleUpldExportClick = type => {
    ...
    window.open(
      `/api/Beauty/beauty/statisticsReport/exportCgAvgPriceBillMaterialDetail?${stringify(params)}`
    );
  };
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({
        type: 'cpstatistics/find_CgAvgPriceBillMaterialDetail',
        payload: {
          ...values,
          pageSize: 10,
          current: 1,
  ...
  render() {
    ...
    const { loading, dispatch, findCgAvgPriceBillMaterialDetaillist } = this.props;
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                scroll={{ x: 1000 }}
                loading={loading}
                data={findCgAvgPriceBillMaterialDetaillist}
                bordered
                columns={columns}
