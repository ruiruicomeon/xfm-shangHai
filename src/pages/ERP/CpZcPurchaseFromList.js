...
import styles from './CpZcPurchaseFromList.less';
      ...
      >
        <div>
          {getFieldDecorator('genTableColumn', {
            initialValue: [],
          })(<SearchTableList searchList={CpZcPurchaseFromSearchList} />)}
        </div>
      </Modal>
    );
  }
...
@connect(({ cpZcPurchaseFrom, loading, cpRevocation }) => ({
  ...cpZcPurchaseFrom,
  ...cpRevocation,
  loading: loading.models.cpZcPurchaseFrom,
}))
@Form.create()

class CpZcPurchaseFromList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpZcPurchaseFrom/cpZcPurchaseFrom_List',
      payload: {
        pageSize: 10,
      }
    });
    ...
    dispatch({
      type: 'cpRevocation/CpZcPurchaseFrom_SearchList',
    });
  }

  gotoForm = () => {
    router.push(`/purchase/process/cp_zc_purchase_from_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/purchase/process/cp_zc_purchase_from_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpZcPurchaseFrom/cpZcPurchaseFrom_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpZcPurchaseFrom/cpZcPurchaseFrom_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  render() {
    ...
    const { loading, cpZcPurchaseFromList, CpZcPurchaseFromSearchList } = this.props;
...
export default CpZcPurchaseFromList;