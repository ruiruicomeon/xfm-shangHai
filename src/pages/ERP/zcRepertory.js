...
@connect(({ cpstatistics, loading ,cpPurchaseDetail,cpOutProduct}) => ({
  ...cpstatistics,
  ...cpPurchaseDetail,
  ...cpOutProduct,
...
handleUpldExportClick = type => {
  const { formValues ,enddate } = this.state;
  ...
  window.open(`/api/Beauty/beauty/cpPurchaseDetail/zcStatisticsExport?${stringify(params)}`);
};


  render() {
    const { selectedRows ,modalVisiblepass ,selectkhflag} = this.state;
    const { loading, zcProuctStatisticslist ,getzcByBilllist,dispatch} = this.props;
    const columns = [
      {
        title: '总成编码',        
      ...
      editable:  true  ,   
      render:(text,res,index)=>{
        if(index == zcProuctStatisticslist.list.length-1){
            return <span>合计数量：{text}</span>
          }
          return <span>{text}</span>
        }   
     }, 
     ...
     },
     {
        ...
        render:(text,res,index)=>{
        if(index == zcProuctStatisticslist.list.length-1){
            return <span>合计金额：{getPrice(text)}</span>
          }
          return <span>{getPrice(text)}</span>
        }
       }, 
        ...
        <div className={styles.standardList}>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator}>
                <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>总成库存报表</div>
              </div>
              <StandardEditTable
                scroll={{ x: 1000 }}
                data={{list:zcProuctStatisticslist.list,pagination:{total:zcProuctStatisticslist.pagination.total,pageSize:zcProuctStatisticslist.pagination.pageSize+1,
                  current:zcProuctStatisticslist.pagination.current ,pageSizeOptions:['11','51','101']}}}
                loading={loading}
