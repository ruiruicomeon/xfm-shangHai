...
class CpCloseList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({
      type: 'cpstatistics/find_OutStorageDetail',
      payload: params,
    });
  };
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`https://xfm-erp.vip/statisticsReport/exportOutStorageDetail?${stringify(params)}`);
  };

  render() {
    const { selectedRows, modalVisiblepass, selectkhflag } = this.state;
    const { loading, cppjstatisticsList, getByBillList, dispatch ,findOutStorageDetaillist} = this.props;
    const columns = [
      {
				title: '单号',         
				dataIndex: 'purchaseId',    
      ...
      // {
      //   title: '发票类型',        
      //   dataIndex: 'inStorageFrom.makeNeed',   
      //   inputType: 'text',
      //   align: 'center',  
      //   width: 100,          
      //   editable: true,      
