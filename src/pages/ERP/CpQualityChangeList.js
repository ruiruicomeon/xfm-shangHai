...
import styles from './CpQualityChangeList.less';
...
@connect(({ cpQualityChange, loading }) => ({
  ...cpQualityChange,
  loading: loading.models.cpQualityChange,
}))
@Form.create()

class CpQualityChangeList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpQualityChange/cpQualityChange_List',
      payload: {
        pageSize: 10,
      }
    });
  ...
  gotoForm = () => {
    router.push(`/business/process/cp_quality_change_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/business/process/cp_quality_change_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpQualityChange/cpQualityChange_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpQualityChange/cpQualityChange_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/cpQualityChange/export?${stringify(params)}`);
    };

  render() {
    const { selectedRows ,array} = this.state;
    const { loading, cpQualityChangeList } = this.props;
...
export default CpQualityChangeList;