...
import styles from './CpStorageList.less';
...
const getValue = obj =>
  Object.keys(obj)
    .map(key => obj[key])
    .join(',');
@connect(({ cpStorage, loading }) => ({
  ...cpStorage,
  loading: loading.models.cpStorage,
}))
@Form.create()
class CpStorageList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  gotoForm = () => {
    router.push(`/basicManagement/warehouse/cp_warehouse/cp_storage_form`);
  }
  gotoUpdateForm = (id) => {
    router.push(`/basicManagement/warehouse/cp_warehouse/cp_storage_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpStorage/cpStorage_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpStorage/cpStorage_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  render() {
    ...
    const { loading, cpStorageList } = this.props;
    const columns = [
     {
        title: '详情',
        width: 100,
 		...
 		},
     {
      ...
      width: 100,
      render: (text, record) => {
        return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='CpStorage').length>0
        && JSON.parse(getStorage('menulist')).filter(item=>item.target=='CpStorage')[0].children.filter(item=>item.name=='删除')
        .length>0?
        <Fragment>
        ...
        <div className={styles.standardList}>
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              {isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='CpStorage').length>0
