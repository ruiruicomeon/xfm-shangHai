...
class CpOfferFormList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpOfferForm/cpSettlement_List',
      payload: {
        pageSize: 10,
        type:2
      }
  ...
  gotoForm = () => {
    router.push(`/business/process/cp_settlement_form_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/business/process/cp_settlement_form_form?id=${id}`);
  }
  ...
  render() {
    ...
    const { loading, cpSettlementList ,cpOfferSearchList} = this.props;
    ...
    let fieldArray = [];
			...
			...fieldArray,
      {
        ...
        align: 'center' , 
        render: (text, record) => {
          return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpSettlementForm').length>0
          && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpSettlementForm')[0].children.filter(item=>item.name=='删除')
          .length>0?
            <Fragment>
              {(isNotBlank(record.orderStatus)&&(record.orderStatus === 0 || record.orderStatus === '0'||record.orderStatus === '未处理')) ?
        ...
        <div className={styles.tableListOperator}>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator}>
                <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>结算单</div>
              </div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 100) + 500 }}
                loading={loading}
                data={cpSettlementList}
                bordered
                columns={columns}
