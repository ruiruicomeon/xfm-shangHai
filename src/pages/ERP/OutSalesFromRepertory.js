...
class CpCloseList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({
      type: 'cpstatistics/find_OutSalesFromDetail',
      payload: params,
    });
  };
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/statisticsReport/exportOutSalesFromDetail?${stringify(params)}`);
  };

  render() {
    const { selectedRows, modalVisiblepass, selectkhflag } = this.state;
    const { loading, cppjstatisticsList, getByBillList, dispatch ,findOutSalesFromDetaillist} = this.props;
    const columns = [
      {
				title: '单号',         
				dataIndex: 'purchaseId',    
      ...
      // {
      //   title: '入库类型',        
      //   dataIndex: 'inStorageFrom.stirageType',   
      //   inputType: 'text',
      //   align: 'center',  
      //   width: 100,          
      //   editable: true,      
      ...
      {
        title: '发票类型',        
        dataIndex: 'outSalesFrom.makeNeed',   
        inputType: 'text',
        align: 'center',  
        width: 100,          
        editable: true,      
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                ...
                loading={loading}
                data={findOutSalesFromDetaillist}
                bordered
                columns={columns}
