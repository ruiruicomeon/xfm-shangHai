...
import {
  Button, Input,  Form, Card, Popconfirm, Icon, Row, Col, Select, Tag,  message, Modal
} from 'antd';
...
import styles from './CpCollecClientList.less';
...
const getValue = obj =>
  Object.keys(obj)
    .map(key => obj[key])
    .join(',');
@connect(({ cpCollecClient, loading }) => ({
  ...cpCollecClient,
  loading: loading.models.cpCollecClient,
}))
@Form.create()
class CpCollecClientList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  gotoForm = () => {
    router.push(`/basicManagement/basis/cp_collec_client_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/basicManagement/basis/cp_collec_client_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpCollecClient/cpCollecClient_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpCollecClient/cpCollecClient_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  render() {
    ...
    const { loading, cpCollecClientList } = this.props;
    const columns = [
     {
        title: '详情',
        width: 100,
...
export default CpCollecClientList;