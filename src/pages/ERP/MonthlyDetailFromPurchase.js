...
class FgsAnalyzeFromPurchase extends PureComponent {
  ...
  handleUpldExportClick = type => {
    ...
    window.open(
      `/api/Beauty/beauty/statisticsReport/exportMonthCgDueMoneyDetail?${stringify(params)}`
    );
  };
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({
        type: 'cpstatistics/find_MonthCgDueMoneyDetail',
        payload: {
          ...values,
          pageSize: 10,
          current: 1,
  ...
  render() {
    ...
    const { loading, dispatch, findMonthCgDueMoneyDetaillist } = this.props;
    const regR = /^R/;
    const regTH = /^(TH)/;
    const regFF = /^(FF)/;
    const regFJ = /^(FJ)/;
    ...
    const columns = [
      ...
      },
      {
        ...
        align: 'center',
        render: text => {
          if (regR.test(text) || regFF.test(text)) {
            return <div>{text}</div>;
          }
          return '';
        },
      },
      {
        ...
        align: 'center',
        render: text => {
          if (regTH.test(text) || regFJ.test(text)) {
            return <div>{text}</div>;
          }
          return '';
        },
      ...
      },
      {
        ...
        render: (text, res) => {
          if (
            isNotBlank(res) &&
            isNotBlank(res.purchaseId) &&
            (regR.test(res.purchaseId) || regTH.test(res.purchaseId))
          ) {
            return <div>{text}</div>;
          }
          return '';
      ...
      },
      {
        ...
        render: (text, res) => {
          if (
            isNotBlank(res) &&
            isNotBlank(res.purchaseId) &&
            (regFF.test(res.purchaseId) || regFJ.test(res.purchaseId))
          ) {
            return <div>{text}</div>;
          }
          return '';
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                scroll={{ x: 1000 }}
                loading={loading}
                data={findMonthCgDueMoneyDetaillist}
                bordered
                columns={columns}
