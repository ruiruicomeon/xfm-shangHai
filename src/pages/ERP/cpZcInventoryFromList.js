...
import styles from './CpZcInventoryFromList.less';
...
@Form.create()
class CpZcInventoryFromList extends PureComponent {
...
class getCpInventoryList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpInventory/getCpInventory_List',
      payload: {
        pageSize: 10,
        type: 1
      }
  ...
  gotoForm = () => {
    router.push(`/warehouse/process/cp_assembly_inventory_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_assembly_inventory_form?id=${id}`);
  }
  ...
  render() {
    ...
    const { loading, getCpInventoryList, CpInStorageFromSearchList } = this.props;
    ...
    const columns = [
      ...
      ...fieldArray,
      {
        ...
        render: (text, record) => {
          ...
          (item=>item.target=='PDZC').length>0&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='PDZC')[0]
    ...
    return (
      ...
      >
        ...
        <CpZcInventoryFromList {...parentSearchMethods} searchVisible={searchVisible} />
      </PageHeaderWrapper>
    );
  }

}
export default getCpInventoryList;