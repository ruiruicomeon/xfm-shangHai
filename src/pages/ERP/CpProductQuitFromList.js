...
import styles from './CpProductQuitFromList.less';
      ...
      render() {
        ...
        return (
          ...
          >
            ...
            initialValue: [],
          })(<SearchTableList searchList={CpProductQuitFromSearchList} />)}
            </div>
          </Modal>
        );
      }
      }
@connect(({ cpProductQuitFrom, loading ,cpRevocation }) => ({
  ...cpProductQuitFrom,
  ...cpRevocation,
  loading: loading.models.cpProductQuitFrom,
}))
@Form.create()
class CpProductQuitFromList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpProductQuitFrom/cpProductQuitFrom_List',
      payload: {
        pageSize: 10,
      }
    });
    dispatch({
      type: 'cpRevocation/CpProductQuitFrom_SearchList',
    });
  }

  gotoForm = () => {
    router.push(`/warehouse/process/cp_product_quit_from_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_product_quit_from_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpProductQuitFrom/cpProductQuitFrom_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpProductQuitFrom/cpProductQuitFrom_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleSearchVisible = (fieldsValue) => {
    ...
    handleFieldChange = (value) => {
      ...
      handleUpldExportClick = type => {
        ...
        window.open(`/api/Beauty/beauty/cpProductQuitFrom/export?${stringify(params)}`);
        };

  render() {
    const { selectedRows ,array,searchVisible} = this.state;
    const { loading, cpProductQuitFromList ,CpProductQuitFromSearchList } = this.props;
    ...
    const field = [
     ...
     {
			title: '仓库',        
			dataIndex: 'storage.entrepotName',   
      inputType: 'text',  
      align: 'center' , 
			width: 100,          
			editable:  true  ,      
    ...
    const columns = [
      {
       ...
       render: (text, record) => {
         ...
         (item=>item.target=='cpProductQuitFrom').length>0&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpProductQuitFrom')[0]
        ...
        <Button icon="cloud-download" onClick={() => this.handleUpldExportClick()}>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>总成退料单</div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 100) + 600 }}
                // selectedRows={selectedRows}
                loading={loading}
                data={cpProductQuitFromList}
                bordered
                columns={columns}
...
export default CpProductQuitFromList;