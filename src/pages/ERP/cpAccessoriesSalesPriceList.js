...
import {
  Button, Input, Form, Card, Popconfirm, Icon,Upload, Row, Col, Select, message, Modal
} from 'antd';
  ...
  Object.keys(obj)
    ...
    const ImportFile = Form.create()(props => {
      ...
      const propsUpload = {
        ...
        fileList: fileL,
        headers: Stoken,
        action: '/api/Beauty/beauty/cpSalesAccessoriesMoney/import',
        beforeUpload(file) {
...
@connect(({ cpAccessoriesSalesPrice, loading }) => ({
  ...cpAccessoriesSalesPrice,
  loading: loading.models.cpAccessoriesSalesPrice,
}))
@Form.create()
class CpBillMaterialList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpAccessoriesSalesPrice/get_CpSalesAccessoriesMoneyList',
      payload: {
        pageSize: 10,
      }
    });
  ...
  gotoForm = () => {
    router.push(`/warehouse/process/cp_accessoriesSales_price_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_accessoriesSales_price_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpAccessoriesSalesPrice/CpSalesAccessoriesMoney_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  handleUpldExportClick = type => {
    const {formValues} = this.state
    const userid = { 'user.id':isNotBlank(getStorage('userid'))?getStorage('userid'):'' };
    window.open(`/api/Beauty/beauty/cpSalesAccessoriesMoney/ExportTemplate?${stringify(userid)}`);
  };

  handleUpldExportClick1 = type => {
    const {formValues} = this.state
    const userid = { ...formValues, 'user.id':isNotBlank(getStorage('userid'))?getStorage('userid'):''  };
    window.open(`/api/Beauty/beauty/cpSalesAccessoriesMoney/ExportTemplate?${stringify(userid)}`);
  };

  render() {
    const { selectedRows ,importFileList, modalImportVisible } = this.state;
    const { loading, getCpSalesAccessoriesMoneyList } = this.props;
    const columns = [
     {
        title: '详情',
        align: 'center' ,
    ...
    //   align: 'center' ,   
 		...
 		// },
     {
      ...
      align: 'center' ,
      render: (text, record) => {
        return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='aPrice').length>0
        && JSON.parse(getStorage('menulist')).filter(item=>item.target=='aPrice')[0].children.filter(item=>item.name=='删除')
        .length>0?
          <Fragment>
        ...
        <div className={styles.standardList}>
          <div className={styles.tableListOperator}>
           ...
           <Button icon="cloud-download" onClick={() => this.handleUpldExportClick(1)}>
            ...
            <Button icon="cloud-download" onClick={() => this.handleUpldExportClick1(0)}>
            导出数据
            </Button>
          </div>
          <Card bordered={false}>
...
配件销售价格表
              ...
              <StandardEditTable
                ...
                loading={loading}
                data={getCpSalesAccessoriesMoneyList}
                bordered
                columns={columns}
