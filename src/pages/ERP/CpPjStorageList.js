...
import styles from './CpPjStorageList.less';
...
const getValue = obj =>
  Object.keys(obj)
    .map(key => obj[key])
    .join(',');
@connect(({ cpPjStorage, loading }) => ({
  ...cpPjStorage,
  loading: loading.models.cpPjStorage,
}))
@Form.create()
class CpPjStorageList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  gotoForm = () => {
    router.push(`/basicManagement/warehouse/pj_warehouse/cp_pj_storage_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/basicManagement/warehouse/pj_warehouse/cp_pj_storage_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpPjStorage/cpPjStorage_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpPjStorage/cpPjStorage_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  render() {
    const { selectedRows ,previewVisible,previewImage} = this.state;
    const { loading, cpPjStorageList } = this.props;
    const columns = [
     {
        title: '详情',
        width: 100,
 		...
 		},
     {
      ...
      align: 'center' ,
      render: (text, record) => {
        return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpPjStorage').length>0
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator} style={{'position':'relative'}}>
                {isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpPjStorage').length>0
&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpPjStorage')[0].children.filter(item=>item.name=='修改')
.length>0&&
<Button icon="plus" type="primary" onClick={() => this.gotoForm()}>
                ...
                {/* {selectedRows.length > 0 && isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpPjStorage').length>0
&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpPjStorage')[0].children.filter(item=>item.name=='修改')
.length>0&&(
<span>
...
export default CpPjStorageList;