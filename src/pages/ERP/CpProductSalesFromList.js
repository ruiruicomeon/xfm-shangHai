...
import styles from './CpProductSalesFromList.less';
      ...
      render() {
        ...
        return (
          ...
          >
            ...
            initialValue: [],
          })(<SearchTableList searchList={CpProductSalesFromSearchList} />)}
            </div>
          </Modal>
        );
      }
      }
@connect(({ cpProductSalesFrom, loading ,cpRevocation}) => ({
  ...cpProductSalesFrom,
  ...cpRevocation,
  loading: loading.models.cpProductSalesFrom,
}))
@Form.create()
class CpProductSalesFromList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpProductSalesFrom/cpProductSalesFrom_List',
      payload: {
        pageSize: 10,
      }
    });
    dispatch({
      type: 'cpRevocation/CpProductSalesFrom_SearchList',
    });
  }

  gotoForm = () => {
    router.push(`/warehouse/process/cp_product_sales_from_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_product_sales_from_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpProductSalesFrom/cpProductSalesFrom_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpProductSalesFrom/cpProductSalesFrom_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleSearchVisible = (fieldsValue) => {
    ...
    handleFieldChange = (value) => {
      ...
      handleUpldExportClick = type => {
        ...
        window.open(`/api/Beauty/beauty/cpProductSalesFrom/export?${stringify(params)}`);
        };

  render() {
    const { selectedRows  ,array,searchVisible} = this.state;
    const { loading, cpProductSalesFromList ,CpProductSalesFromSearchList} = this.props;
    ...
    const columns = [
       ...
       ...fieldArray,
      {
       ...
       render: (text, record) => {
         ...
         (item=>item.target=='cpProductSalesFrom').length>0&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpProductSalesFrom')[0]
        ...
        <Button icon="cloud-download" onClick={() => this.handleUpldExportClick()}>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>总成退货单</div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 100) + 600 }}
                // selectedRows={selectedRows}
                loading={loading}
                data={cpProductSalesFromList}
                bordered
                columns={columns}
...
export default CpProductSalesFromList;