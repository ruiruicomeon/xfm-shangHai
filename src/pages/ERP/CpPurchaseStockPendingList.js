...
import styles from './CpPurchaseStockPendingList.less';
      ...
      >
        <div>
          {getFieldDecorator('genTableColumn', {
            initialValue: [],
          })(<SearchTableList searchList={CpPurchaseStockPendingSearchList} />)}
        </div>
      </Modal>
    );
  }
...
@Form.create()
class CpPurchaseStockPendingList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpRevocation/CpPurchaseStockPending_SearchList',
    });
  }
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    this.setState({
      xcurrent: pagination.current,
      xpageSize: pagination.pageSize
    })
  ...
  handleDeleteClick = (id) => {
    const { dispatch } = this.props;
    const { selectedRows, formValues, xcurrent, xpageSize } = this.state;
    let ids = '';
    if (isNotBlank(id)) {
      ids = id;
    } else {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpPurchaseStockPending/cpPurchaseStockPending_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  handleAddpass = (val) => {
    const { dispatch } = this.props
    const { rukuid, neiparent, xcurrent, xpageSize } = this.state
    const values = { ...val }
    // if(values.isOll==1){
    ...
    if (neiparent == -1) {
      dispatch({
        type: 'cpPurchaseStockPending/post_NB_CpPurchaseStockPending',
        payload: {
          id: rukuid,
          ...values,
        },
  ...
  editAndSwitch = (checked, record) => {
    if (isNotBlank(record) && isNotBlank(record.id) && isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpPurchaseStockPending').length > 0
      && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpPurchaseStockPending')[0].children.filter(item => item.name == '修改')
        .length > 0) {
      Modal.confirm({
        title: `修改启用状态`,
  ...
  checkSwitch = (checked, record) => {
    const { dispatch } = this.props
    const { xcurrent, xpageSize } = this.state
    dispatch({
      type: 'cpPurchaseStockPending/cpPurchaseStockPending_Update',
      payload: {
        id: record.id,
  ...
  render() {
    ...
    const { loading, cpPurchaseStockPendingList, CpPurchaseStockPendingSearchList } = this.props;
    ...
    const field = [
      ...
      },
      {
        ...
        render: (text, record) => {
          if (isNotBlank(text)) {
            return <Switch
              disabled={!(isNotBlank(record) && isNotBlank(record.id) && isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpPurchaseStockPending').length > 0
                && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpPurchaseStockPending')[0].children.filter(item => item.name == '修改')
                  .length > 0)}
    ...
    const columns = [
      {
        ...
        fixed: 'left',
        render: (text, record) => {
          return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpPurchaseStockPending').length > 0
            && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpPurchaseStockPending')[0].children.filter(item => item.name == '修改')
              .length > 0 ?
            <Fragment>
              {
                isNotBlank(record) && record.orderStatus != 2 && record.orderStatus != '已关闭' &&
                <a onClick={() => this.shownumber(record.id, record)}>入库</a>
              }
            </Fragment>
            : ''
        },
        ...
        <div className={styles.standardList}>
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div style={{ textAlign: 'center', fontWeight: 550, fontSize: 28, marginBottom: '16px' }}>待入库清单</div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 100) + 500 }}
                loading={loading}
                data={cpPurchaseStockPendingList}
                bordered
                columns={columns}
...
export default CpPurchaseStockPendingList;