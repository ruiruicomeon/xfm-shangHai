...
import styles from './CpOutSalesFromList.less';
      ...
      render() {
        ...
        return (
          ...
          >
            ...
            initialValue: [],
          })(<SearchTableList searchList={CpOutSalesFromSearchList} />)}
            </div>
          </Modal>
        );
      }
      }
@connect(({ cpOutSalesFrom, loading ,cpRevocation}) => ({
  ...cpOutSalesFrom,
  ...cpRevocation,
  loading: loading.models.cpOutSalesFrom,
}))
@Form.create()
class CpOutSalesFromList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    if(isNotBlank(location.state)&&isNotBlank(location.state.id)){
      dispatch({
        type: 'cpOutSalesFrom/cpOutSalesFrom_List',
        payload: {
          pageSize: 10,
    ...
    dispatch({
      type: 'cpRevocation/CpOutSalesFrom_SearchList',
    });
  }

  gotoForm = () => {
    router.push(`/warehouse/process/cp_out_sales_from_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_out_sales_from_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpOutSalesFrom/cpOutSalesFrom_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpOutSalesFrom/cpOutSalesFrom_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleSearchVisible = (fieldsValue) => {
    ...
    handleFieldChange = (value) => {
      ...
      handleUpldExportClick = type => {
        ...
        window.open(`/api/Beauty/beauty/cpOutSalesFrom/export?${stringify(params)}`);
        };
  ...
  render() {
    ...
    const { loading, cpOutSalesFromList ,CpOutSalesFromSearchList } = this.props;
    ...
    const columns = [
       ...
       ...fieldArray,
      {
       ...
       render: (text, record) => {
         ...
         (item=>item.target=='cpOutSalesFrom').length>0&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpOutSalesFrom')[0]
...
export default CpOutSalesFromList;