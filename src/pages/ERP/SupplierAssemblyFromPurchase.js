...
@Form.create()
class QuitFromPurchase extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  handleUpldExportClick = type => {
    ...
    window.open(
      `/api/Beauty/beauty/statisticsReport/exportCgAssemblyBuildDetail?${stringify(params)}`
    );
  };
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({
        type: 'cpstatistics/find_CgAssemblyBuildDetail',
        payload: {
          ...values,
          pageSize: 10,
          current: 1,
  ...
  render() {
    ...
    const { loading, dispatch, findCgAssemblyBuildDetaillist } = this.props;
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                scroll={{ x: 1000 }}
                loading={loading}
                data={findCgAssemblyBuildDetaillist}
                bordered
                columns={columns}
...
export default QuitFromPurchase;
