...
class CpCloseList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({
      type: 'cpstatistics/find_ZCOutStorageDetail',
      payload: params,
    });
  };
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/statisticsReport/exportZCOutStorageDetail?${stringify(params)}`);
  };

  render() {
    const { selectedRows, modalVisiblepass, selectkhflag } = this.state;
    const { loading, cppjstatisticsList, getByBillList, dispatch ,findZCOutStorageDetaillist} = this.props;
    const columns = [
      {
				title: '单号',         
				dataIndex: 'purchaseId',    
      ...
      {
        title: '出库类型',        
        dataIndex: 'outProduct.zcStirageType',   
        inputType: 'text',
        align: 'center',  
        width: 100,          
        editable: true,      
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                ...
                loading={loading}
                data={findZCOutStorageDetaillist}
                bordered
                columns={columns}
