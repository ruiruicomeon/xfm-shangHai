...
const CreateFormcf = Form.create()(props => {
  const { selectcfflag, form, handleAddcf, handleModalVisiblecf, form: { getFieldDecorator }, onselecthis, selcfdata, selecthisdata } = props;
  const okHandle = () => {
    form.validateFields((err, fieldsValue) => {
      form.resetFields();
      const values = { ...fieldsValue };
...
@connect(({ cpStartInvoice, loading, cpOfferForm, cpClient, cpCollecClient, syslevel, sysdept, sysuser, cpAfterApplicationFrom }) => ({
  ...
  submitting1: loading.effects['cpStartInvoice/update_CpStartInvoice'],
}))
@Form.create()
class CpStartInvoiceForm extends PureComponent {
  ...
  handleSubmit = e => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        // 	value.collectClientId.id = selectjcdata.id
        // }
        // value.sendDate = moment(value.sendDate).format("YYYY-MM-DD")
        // value.orderStatus = 1
        ...
        dispatch({
          type: 'cpStartInvoice/update_CpStartInvoice',
          payload: {
            id: isNotBlank(location.query) && isNotBlank(location.query.newid) ? location.query.newid : '',
          ...
          callback: () => {
            ...
            router.push('/business/process/cp_start_invoice_already_list');
          }
        })
      }
    });
  ...
  handleDeleteClickhb1 = (id, money) => {
    ...
    if (delflag) {
      ...
      const newinreceivablemoney = inreceivablemoney - money
  ...
  onshowdd = () => {
    ...
    const { cpStartInvoiceGet, dispatch } = this.props
  ...
  handleAddcodemore = () => {
    ...
    const { selectedRows, location, selflag, valuetype, newarr } = this.state
    let ids = '';
    ...
    // if(!isNotBlank(location.query.id)&&isNotBlank(selectCompleteList.list)&&selectCompleteList.list.length>0){
    let newarrcopy1 = []
    ...
    newarrcopy1.forEach((item, idx) => {
      if (!isNotBlank(item.duty)) {
        newarrcopy1[idx].duty = item.orderCode
      }
      if (!isNotBlank(item.invoiceMoney)) {
        newarrcopy1[idx].receivableMoney = item.money
      }
      if (!isNotBlank(item.invoiceMoney)) {
        newarrcopy1[idx].invoiceMoney = item.money
      }
      if (!isNotBlank(item.account)) {
        newarrcopy1[idx].account = item.id
      }
    })
  ...
  selectcustomer = (record) => {
    ...
    dispatch({
      ...
      // 		callback: (resin) => {
      // 			that.setState({
      // 				inreceivablemoney: isNotBlank(resin) ? resin : 0
      // 			})
      // 		}
      // 	});
      // 	}
  ...
  handleSearchAddmore = (fieldsValue) => {
    const { dispatch } = this.props;
    const { selflag, clientcollectId } = this.state
    if (isNotBlank(selflag) && selflag == 'kh') {
      dispatch({
        type: 'cpAfterApplicationFrom/select_complete_List',
        payload: {
  ...
  render() {
    const { fileList, previewVisible, previewImage, selectcodeflag, selectcodedata, selecthisflag, selecthisdata,
      ...
      selectmoreflag, selectedRows, allmoney, searchVisiblecode, searchVisiblemore, selflag, clientcollectId, inreceivablemoney
      , maintenance_project, modalVisiblepass, showdata, ordermoney, newarr, location, otherclientid, historytype, searchVisiblekh } = this.state;
    const { submitting1, submitting, cpStartInvoiceGet, selectCompleteList, cpStartInvoiceNotPageList, getCpStartInvoiceListCpcliect, cpOfferSearchList, cpOfferFormList,
    ...
    const columnscf = [
      {
        ...
        render: (text, record) => {
          if (!orderflag) {
            return <Fragment>
              <a onClick={() => this.editcfmx(record)}>修改</a>
            </Fragment>
          }
        },
      },
    ...
    const columnshb1 = [
      ...
      },
      {
        ...
        render: (text, record) => {
          if (!orderflag) {
            return <Fragment>
              <Popconfirm title="是否确认删除本行?" onConfirm={() => this.handleDeleteClickhb1(record.id, record.invoiceMoney)}>
                <a>删除</a>
              </Popconfirm>
            </Fragment>
          }
            ...
            <FormItem {...formItemLayout} label='审批进度'>
                    ...
                    <FormItem {...formItemLayout} label="拟定回款日期">
                      {getFieldDecorator('endDate', {
                        initialValue: isNotBlank(selectcodedata) && isNotBlank(selectcodedata.returnedDate) ? selectcodedata.returnedDate :
                          isNotBlank(cpStartInvoiceGet) && isNotBlank(cpStartInvoiceGet.endDate) ? cpStartInvoiceGet.endDate : '',
                      })(
                        <Input disabled />
                      )}
                    </FormItem>
              ...
              <Col lg={24} md={24} sm={24}>
                <FormItem {...formItemLayout} label="备注信息" className="allinputstyle">
                  {getFieldDecorator('remarks', {
                    initialValue: isNotBlank(location) && isNotBlank(location.query) && isNotBlank(location.query.remarks) ? location.query.remarks : '',
                    rules: [
                      {
                        required: false,
                        message: '请输入备注信息',
...
>
          ...
          <div style={{ textAlign: 'center', marginTop: '15px' }}>
            ...
            <FormItem {...submitFormLayout} style={{ marginTop: 32, textAlign: 'center' }}>
              {/* {isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpStartInvoice').length > 0
								&& JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpStartInvoice')[0].children.filter(item => item.name == '修改')
									.length > 0 && <span>
  <Button loading={submitting1} type="primary" onClick={this.onsave} disabled={orderflag}>
										保存
  </Button>
  <Button
    style={{ marginLeft: 8 }}
  ...
  {isNotBlank(cpStartInvoiceGet) && isNotBlank(cpStartInvoiceGet.approvals) &&
                      ...
                      <Button style={{ marginLeft: 8 }} loading={submitting1} onClick={() => this.onUndorepost()}>
                        重新提交
                      </Button>
                    }
  {
              ...
              <Button style={{ marginLeft: 8 }} type="primary"
                htmlType="submit" loading={submitting1} >
                提交
          </Button>
