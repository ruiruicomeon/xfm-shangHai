...
const ImportFile = Form.create()(props => {
  ...
  const propsUpload = {
    ...
    fileList: fileL,
    headers: Stoken,
    action: '/api/Beauty/beauty/cpPurchaseDetail/importPurchase',
    beforeUpload(file) {
...
@connect(({ cpInStorageFrom, loading, cpBillMaterial, cpPurchaseDetail, cpPjEntrepot, cpPjStorage, company, cpInventory }) => ({
  ...cpInStorageFrom,
  ...cpBillMaterial,
  ...cpPurchaseDetail,
  ...cpPjEntrepot,
  ...
  submitting2: loading.effects['cpInventory/save_CpInventory'],
}))
@Form.create()
class CpInStorageFromForm extends PureComponent {
      ...
      dispatch({
        ...
        callback: (res) => {
          if (res.data.status === 1 || res.data.status === '1' || res.data.status === 2 || res.data.status === '2'
            || (isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'PD').length > 0
              && JSON.parse(getStorage('menulist')).filter(item => item.target == 'PD')[0].children.filter(item => item.name == '修改')
                .length == 0)) {
          ...
          this.props.form.setFieldsValue({
            ck: isNotBlank(res) && isNotBlank(res.data) && isNotBlank(res.data.entrepotName) ? res.data.entrepotName : ''
          })
          if (isNotBlank(res) && isNotBlank(res.data) && isNotBlank(res.data.cpPjEntrepot) && isNotBlank(res.data.cpPjEntrepot.id)) {
            ...
            dispatch({
              type: 'cpPurchaseDetail/cpInventory',
              payload: {
      ...
      if (!err) {
        if (isNotBlank(cpPurchaseDetailList) && isNotBlank(cpPurchaseDetailList.list) && cpPurchaseDetailList.list.length > 0 && cpPurchaseDetailList.list.some((item) => { return item.billtype == 0 })) {
          ...
          if (isNotBlank(location.query) && isNotBlank(location.query.id)) {
            ...
            value.status = 1
            dispatch({
              type: 'cpInventory/save_CpInventory',
              payload: { ...value },
              callback: (res) => {
                this.setState({
                  addfileList: [],
                ...
                if (isNotBlank(res) && isNotBlank(res.data) && isNotBlank(res.data.id)) {
                  router.push(`/warehouse/process/cp_pjInventory_from_form?id=${res.data.id}`);
                } else {
                  // router.push(`/warehouse/process/cp_in_storage_from_form?id=${location.query.id}`);
                  router.push('/warehouse/process/cp_pjInventory_from_list');
                }
              }
            })
          }
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        if (updataflag) {
          value.status = 0
          dispatch({
            type: 'cpInventory/post_CpInventory',
            payload: { ...value },
            callback: (res) => {
              router.push(`/warehouse/process/cp_pjInventory_from_form?id=${res.data.id}`);
            }
  ...
  handleFormAdd = (values) => {
    ...
    dispatch({
      type: 'cpInventory/post_CpInventory_OperationItem',
      payload: {
        type: 1,
  ...
  undoClick = id => {
    ...
    dispatch({
      type: 'cpRevocation/undo_Inventory',
      payload: {
        id
      },
      callback: () => {
  ...
  handleUpldExportClick = (type) => {
    ...
    window.open(`/api/Beauty/beauty/cpPurchaseDetail/exportPurchase?${stringify(userid)}`);
  };
  ...
  render() {
    ...
    const { submitting2, getCpInventorywllist, submitting, submitting1, complist, CpInventoryGet, cpSupplierList, cpBillMaterialList, cpBillMaterialMiddleList, cpPurchaseDetailList, cpPjEntrepotList, cpPjStorageList, cpPurchaseDetailModalList, dispatch } = this.props;
    const {
    ...
    const columns = [
      ...
      {
        title: '一级编码型号',        
        dataIndex: 'billmaterialOnecodemodel',
        inputType: 'text',
        align: 'center',   
        width: 100,          
        editable: true,      
      ...
      {
        title: '原厂编码',        
        dataIndex: 'billmaterialOriginalCode',   
        inputType: 'text',
        align: 'center',    
        width: 100,          
        editable: true,      
    ...
    return (
      <PageHeaderWrapper>
        <Card bordered={false}>
          <div style={{ fontWeight: 550, fontSize: 28, textAlign: 'center' }}>配件盘点单</div>
            ...
            <Card title="基本信息" className={styles.card} bordered={false}>
              ...
              <FormItem {...submitFormLayout} style={{ marginTop: 32, textAlign: 'center' }}>
                {isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'PD').length > 0
                  && JSON.parse(getStorage('menulist')).filter(item => item.target == 'PD')[0].children.filter(item => item.name == '修改')
                    .length > 0 && <span>
                      <Button type="primary" loading={submitting2} style={{ marginLeft: 8 }} onClick={this.onsave} disabled={orderflag && updataflag}>
                      保存
                      </Button>
                      {
