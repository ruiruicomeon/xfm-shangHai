...
class CpCloseList extends PureComponent {
  ...
  componentDidMount() {
    ...
    // dispatch({
    //   type: 'cpstatistics/get_pj_plant',
    //   payload: {
    //     pageSize: 10,
    //   }
    // });
...
handleUpldExportClick = type => {
  ...
  const newkey =  isNotBlank(newobj.cpBillMaterial)&&isNotBlank(newobj.cpBillMaterial.billCode)?newobj.cpBillMaterial.billCode:''
  ...
  const params = {
    ...
    startDate: startdate,
    ...newobj,
    'cpBillMaterial.billCode':newkey,
    'user.id':getStorage('userid'),
  }
  window.open(`/api/Beauty/beauty/cpPurchaseDetail/pjplantExport?${stringify(params)}`);
};


  render() {
    const { selectedRows ,modalVisiblepass ,selectkhflag} = this.state;
    const { loading, pjplantlist ,getzcByBilllist,dispatch} = this.props;
    const columns = [
       {
        title: '操作分类',        
        dataIndex: 'remarks',   
        ...
        <div className={styles.standardList}>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator}>
                <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>配件车间现场表</div>
              </div>
              <StandardEditTable
                scroll={{ x: 1000 }}
                loading={loading}
