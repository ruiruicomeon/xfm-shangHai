...
import styles from './CpAccessoriesSalesFormList.less';
...
@connect(({ cpAccessoriesSalesForm, loading }) => ({
  ...cpAccessoriesSalesForm,
  loading: loading.models.cpAccessoriesSalesForm,
}))
@Form.create()
class CpAccessoriesSalesFormList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpAccessoriesSalesForm/cpAccessoriesSalesForm_List',
      payload: {
        pageSize: 10,
      }
    });
  ...
  gotoForm = () => {
    router.push(`/business/process/cp_accessories_sales_form_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/business/process/cp_accessories_sales_form_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpAccessoriesSalesForm/cpAccessoriesSalesForm_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpAccessoriesSalesForm/cpAccessoriesSalesForm_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/cpAccessoriesSalesForm/export?${stringify(params)}`);
  };
  ...
  render() {
    ...
    const { loading, cpAccessoriesSalesFormList, CpAccessoriesSalesSearchList } = this.props;
    ...
    const field = [
      ...
      },
      {
        ...
        editable: true,
        render: (text, res) => {
          if (isNotBlank(res.type) && res.type == 5) {
            return ''
          }
          return text
        }
    ...
    const columns = [
      ...
      ...fieldArray,
      {
        ...
        align: 'center',
        render: (text, record) => {
          return ((record.orderStatus === 0 || record.orderStatus === '0' || record.orderStatus === '未处理') && isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpAccessoriesSalesForm').length > 0
            && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpAccessoriesSalesForm')[0].children.filter(item => item.name == '删除')
              .length > 0) ?
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator} style={{ 'position': 'relative' }}>
                {
                  isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpAccessoriesSalesForm').length > 0
                    && JSON.parse(getStorage('menulist')).filter(item => item.target == 'cpAccessoriesSalesForm')[0].children.filter(item => item.name == '修改').length > 0 ?
              ...
              <StandardEditTable
                ...
                loading={loading}
                data={cpAccessoriesSalesFormList}
                bordered
                columns={columns}
...
export default CpAccessoriesSalesFormList;