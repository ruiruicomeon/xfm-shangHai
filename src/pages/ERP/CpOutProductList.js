...
import styles from './CpOutProductList.less';
      ...
      render() {
        ...
        return (
          ...
          >
            ...
            initialValue: [],
          })(<SearchTableList searchList={CpOutProductSearchList} />)}
            </div>
          </Modal>
        );
      }
      }
@connect(({ cpOutProduct, loading ,cpRevocation }) => ({
  ...cpOutProduct,
  ...cpRevocation,
  loading: loading.models.cpOutProduct,
}))
@Form.create()
class CpOutProductList extends PureComponent {
  state = {
    expandForm: false,
    selectedRows: [],
    formValues: {},
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'cpOutProduct/cpOutProduct_List',
      payload: {
        pageSize: 10,
      }
    });
    dispatch({
      type: 'cpRevocation/CpOutProduct_SearchList',
    });
  }

  gotoForm = () => {
    router.push(`/warehouse/process/cp_out_product_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/warehouse/process/cp_out_product_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpOutProduct/cpOutProduct_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpOutProduct/cpOutProduct_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/cpOutProduct/export?${stringify(params)}`);
    };

  render() {
    const { selectedRows ,array,searchVisible} = this.state;
    const { loading, cpOutProductList ,CpOutProductSearchList } = this.props;
    ...
    const columns = [
       ...
       ...fieldArray,
      {
       ...
       render: (text, record) => {
         ...
         (item=>item.target=='cpOutProduct').length>0&& JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpOutProduct')[0]
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator}>
                <div style={{textAlign:'center',fontWeight:550,fontSize:28,marginBottom:'16px'}}>总成出库单</div>
              </div>
              <StandardEditTable
                scroll={{ x: (fieldArray.length * 100) + 500 }}
                // selectedRows={selectedRows}
                loading={loading}
                data={cpOutProductList}
                bordered
                columns={columns}
...
export default CpOutProductList;