...
class CpCloseList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({
      type: 'cpstatistics/find_QuitFromDetail',
      payload: params,
    });
  };
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`https://xfm-erp.vip/statisticsReport/exportQuitFromDetail?${stringify(params)}`);
  };

  render() {
    const { selectedRows, modalVisiblepass, selectkhflag } = this.state;
    const { loading, cppjstatisticsList, getByBillList, dispatch ,findQuitFromDetaillist} = this.props;
    const columns = [
      {
				title: '单号',         
				dataIndex: 'purchaseId',    
