...
import {
  Button, Input, InputNumber, Form, Card, Popconfirm, Icon, Row,Upload, Col, Select, DatePicker, Divider, Tag, Avatar, message, Modal
} from 'antd';
...
import styles from './CpTwoCodeList.less';
...
const ImportFile = Form.create()(props => {
  ...
  const propsUpload = {
    ...
    fileList: fileL,
    headers: Stoken,
    action: '/api/Beauty/beauty/cpTwoCode/import',
...
@connect(({ cpTwoCode, loading }) => ({
  ...cpTwoCode,
  loading: loading.models.cpTwoCode,
}))
@Form.create()

class CpTwoCodeList extends PureComponent {
  ...
  gotoForm = () => {
    router.push(`/basicManagement/materials/cp_two_code_form`);
  }

  gotoUpdateForm = (id) => {
    router.push(`/basicManagement/materials/cp_two_code_form?id=${id}`);
  }
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        type: 'cpTwoCode/cpTwoCode_Delete',
        payload: {
          id: ids
        },
        callback: () => {
  ...
  onSaveData = (key, row) => {
    ...
    if (isNotBlank(value)) {
      ...
      dispatch({
        type: 'cpTwoCode/cpTwoCode_Update',
        payload: {
          id: key,
          ...value
        },
  ...
  render() {
    ...
    const { loading, cpTwoCodeList } = this.props;
    const columns = [
     {
        title: '详情',
        width: 100,
 		...
 		},
     {
      ...
      width: 100,
      render: (text, record) => {
        return isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpTwoCode').length>0
        && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpTwoCode')[0].children.filter(item=>item.name=='删除')
        .length>0?
          <Fragment>
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator} style={{'position':'relative'}}>
                ...
                {isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item=>item.target=='cpTwoCode').length>0
