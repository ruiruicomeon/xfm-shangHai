...
import {
  Button, Input, Form, Card, Icon, Upload, Row, Col, Select, DatePicker, message, Modal
} from 'antd';
...
const CreateFormMore = Form.create()(props => {
  ...
  getCpBillMaterialListNo2, selectmore, handleSelectRows1, selectedRows1, dispatch, form  ,location} = props;
  const selectcolumns = [
    {
      title: '物料编码',        
      dataIndex: 'billCode',   
  ...
  const handleSearch = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({
        type: 'cpClient/get_CpBillMaterial_ListNo2',
        payload: {
          pageSize: 100,
          ...values,
...
class CpBillMaterialListNew extends PureComponent {
  ...
  handleDeleteClick = (id) => {
    ...
    if (isNotBlank(ids)) {
      dispatch({
        ...
        // type: 'cpClient/update_Must_Parent',
        payload: {
          ids,
          typeId:location.query.id,
        },
  ...
  render() {
    ...
    const { loading, getCpBillMaterialListNo2,getCpBillMaterialListNo1, cpBillMaterialList, dispatch, getcpBillmaterialList1, getcpBillmaterialList2 } = this.props;
    console.log(getCpBillMaterialListNo2)
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <div className={styles.tableListOperator} style={{ 'position': 'relative' }}>
                ...
                {selectedRows.length > 0 && isNotBlank(getStorage('menulist')) && JSON.parse(getStorage('menulist')).filter(item => item.target == 'mutest').length > 0
                  && JSON.parse(getStorage('menulist')).filter(item => item.target == 'mutest')[0].children.filter(item => item.name == '修改')
                    .length > 0 && (
                  <span>
                ...
                <span style={{ fontWeight: 550, fontSize: 28, position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)' }}>
                  必须件物料({isNotBlank(location.query)&&isNotBlank(location.query.name)?location.query.name:''})
                </span>
              </div>
              <StandardEditTable
                scroll={{ x: 700 }}
