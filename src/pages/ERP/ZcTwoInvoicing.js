...
class CpCloseList extends PureComponent {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    dispatch({
      type: 'cpstatistics/find_AssemblyTwoInStock',
      payload: params,
    });
  };
  ...
  handleUpldExportClick = type => {
    ...
    window.open(`/api/Beauty/beauty/statisticsReport/exportAssemblyTwoInStock?${stringify(params)}`);
  };

  render() {
    const { selectedRows, modalVisiblepass, selectkhflag } = this.state;
    const { loading, cppjstatisticsList, getByBillList, dispatch ,findAssemblyTwoInStocklist} = this.props;
    const columns = [
      {
        title: '总成编码',        
      ...
      {
        title: '入库供应商',        
        dataIndex: 'product.cpSupplier.name',   
        inputType: 'text',   
        width: 150,          
        editable: true,      
      },
      ...
      {
        title: '出库日期',        
        dataIndex: 'outProduct.finishDate',   
        inputType: 'text',   
        width: 150,          
        editable: true,
        render: (val)=>{
      ...
      {
        title: '订单编号',        
        dataIndex: 'outProduct.orderCode',   
        inputType: 'text',   
        width: 200,          
        editable: true,      
      },
      {
        title: '出库客户名',        
        dataIndex: 'outProduct.clientName',   
        inputType: 'text',   
        width: 150,          
        editable: true,      
      },
      ...
      {
        title: '退料日期',        
        dataIndex: 'pQuitFrom.finishDate',   
        inputType: 'text',   
        width: 150,          
        editable: true,
        render: (val)=>{
      ...
      {
        title: '退货日期',        
        dataIndex: 'productSales.finishDate',   
        inputType: 'text',   
        width: 150,          
        editable: true,
        render: (val)=>{
      ...
      {
        title: '订单编号',        
        dataIndex: 'productSales.orderCode',   
        inputType: 'text',   
        width: 200,          
        editable: true,      
      },
      ...
      {
        title: '退货供应商',        
        dataIndex: 'productSales.cpSupplier.name',   
        inputType: 'text',   
        width: 150,          
        editable: true,      
      },
      {
        title: '盘盈数量',        
        dataIndex: 'pyOperation.pyNumber',   
        inputType: 'text',   
        width: 100,          
        editable: true,      
      },
      ...
      {
        title: '盘盈日期',        
        dataIndex: 'pyOperation.finishDate',   
        inputType: 'text',   
        width: 150,          
        editable: true,
        render: (val)=>{
      ...
      {
        title: '盘亏日期',        
        dataIndex: 'pkOperation.finishDate',   
        inputType: 'text',   
        width: 150,          
        editable: true,
        render: (val)=>{
          ...
          <Card bordered={false}>
            <div className={styles.tableList}>
              ...
              <StandardEditTable
                ...
                data={{
                  list:
                    isNotBlank(findAssemblyTwoInStocklist) &&
                    isNotBlank(findAssemblyTwoInStocklist.list) &&
                    findAssemblyTwoInStocklist.list.length > 0
                      ? findAssemblyTwoInStocklist.list
                      : [],
                  pagination: {
                    total:
                      isNotBlank(findAssemblyTwoInStocklist) &&
                      isNotBlank(findAssemblyTwoInStocklist.total)
                        ? findAssemblyTwoInStocklist.total
                        : 0,
                    current:
                      isNotBlank(findAssemblyTwoInStocklist) &&
                      isNotBlank(findAssemblyTwoInStocklist.current)
                        ? findAssemblyTwoInStocklist.current
                        : 1,
                    pageSize:
                      isNotBlank(findAssemblyTwoInStocklist) &&
                      isNotBlank(findAssemblyTwoInStocklist.pageSize)
                        ? findAssemblyTwoInStocklist.pageSize
                        : 10,
                  },
                }}
                loading={loading}
